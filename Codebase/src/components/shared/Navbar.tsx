...
function PillarIcon({ icon, className }: { icon: string; className?: string }) {
  ...
  if (icon === "core") {
    return (
      <svg width={size} height={size} viewBox="0 0 28 28" fill="none" className={className}>
        <path d="M14 4L24 22H4L14 4Z" fill="currentColor" />
      </svg>
    );
  }
  if (icon === "code") {
  ...
  if (icon === "scale") {
    return (
      <svg width={size} height={size} viewBox="0 0 28 28" fill="none" className={className}>
        <path d="M14 24L4 6H24L14 24Z" fill="currentColor" />
      </svg>
    );
  }
  // style
  return (
    <span
      className={`inline-flex items-center justify-center font-display text-[13px] leading-none ${className}`}
